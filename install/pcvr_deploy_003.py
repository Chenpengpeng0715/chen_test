...
class PCVRAssistantTestCase(unittest.TestCase):
    ...
    def test_app_install_003(self):
        # 点击【确定】
        self.screen.click(image_config.Install_path.terminate_install_confirm_path)
        time.sleep(1)
        self.assertFalse(self.screen.exists(image_config.Install.install_home_page_path))
        '''
        # 点击【取消】
        self.screen.click(image_config.Install_path.terminate_install_cancel_path)
        self.assertTrue(self.screen.exists(image_config.Install.install_home_page_path))
        '''
    def setUp(self):
        """
