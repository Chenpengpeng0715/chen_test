...
def main():
	...
	#测试结果保存父路径
	dest_path = r"D:\fanyifu\result\test\B122"
	#获取devices列表
	...
	for i in range(len(file_list)):
		...
		start_server(people_path, push_path, pull_path, result_path, sn)
...
def FIUT(people_path, push_path, pull_path, result_path, sn):
	...
	logcmd = "adb -s " + sn + " logcat -s FIUTCA"
...
def start_server(people_path, push_path, pull_path, result_path, sn):
	server_sn = threading.Thread(target = FIUT, args = (people_path, push_path, pull_path, result_path, sn))
	server_sn.start()
